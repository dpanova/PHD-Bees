...
class Bee:
    ...
    def new_y_label_creation(self, old_col=['missing queen', 'queen',
        ...
        if type(old_col) != list:
            raise ValueError(
                'old_col input is not the correct type. It is type %s, but it should be a list.' % type(old_col))
        if type(new_col) != str:
            raise ValueError(
                'new_col input is not the correct type. It is type %s, but it should be a str.' % type(new_col))
    ...
    def split_annotation_data(self, perc=0.25, no_bee=False, data_quality=True, stratified=True):
        ...
        if type(no_bee) != bool:
            raise ValueError(
                'no_bee input is not the correct type. It is type %s, but it should be a boolean.' % type(no_bee))
        ...
        if self.y_col not in self.annotation_df:
            ...
            if data_quality:
                ...
                self.annotation_df = self.annotation_df.merge(existing_indices, how='left', left_on='index',
                                                              right_on='index')
    ...
    def harley_transformation_with_window(self, x, window=np.hanning):
        ...
        if window not in window_list:
            ...
            logging.info(
                'Harley transformation is completed for the array conducted with window function %s' % window.__name__)
        return x_trans
    ...
    def freq_powerspect_func(self, x, dt, npnts, denoise=True, cutoff=0):
        ...
        if type(x) != np.ndarray:
            ...
            raise ValueError(
                'Invalid denoise type. denoise is type %s and expected type is bool.' % type(denoise).__name__)
    ...
    def binning(self, x, dt, npnts, n_bins=128, n_start=0.0, n_end=2000.0, denoise=True, cutoff=0):
        ...
        if type(x) != np.ndarray:
            ...
            raise ValueError(
                'Invalid n_start type. n_start is type %s and expected type is float.' % type(n_start).__name__)
    ...
    def data_transformation_row(self, arg):
        ...
        file_index = row[
            self.x_col]  # this index is necessary to ensure we have the correct file name (coming from the annotation file)
    ...
    def data_transformation_df(self, X):
        ...
        if type(X) != pd.core.frame.DataFrame:
            raise ValueError(
                'Invalid arg type. arg is type %s and expected type is pandas.core.frame.DataFrame.' % type(X).__name__)
        ...
        X_transformed = pool.map(self.data_transformation_row,
                                 [(train_index, row) for train_index, row in X.iterrows()])
    ...
    def model_results(self
        ...
        try:
            ...
            y_pred = rand_search.predict(
                self.X_test[[x for x in self.X_test.columns if x not in ['train_index', 'file_index']]])
    ...
    def random_forest_results(self):
        ...
        try:
            ...
            self.forest_importances = pd.Series(importances, index=[x for x in self.X_train.columns if
